...
2. **`<PRODUTOS ATUAIS PARA CLIENTES EXTERNOS>` (optional)**: Current external-facing products or services — used to inspire integration or expansion ideas.
3. **`<FERRAMENTAS ATUAIS INTERNAS DA ORGANIZAÇÃO>` (optional)**: Internal tools/platforms — used only when the input relates to internal organizational changes.
