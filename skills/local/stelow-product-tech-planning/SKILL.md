...
```
- ✅ `product_type: software` or `product_type: hybrid` → activate stelow-product-testing-ai-code
...
**See also:**
- `skills/stelow-product-scope-executor/SKILL.md#3e-ter` — task seeding, append, mark-done.
- `skills/stelow-product-tech-planning/references/scopes-and-sequencing.md#Scope Detail Template` — the markdown table format.
...
1. **Generate testing-strategy.md via subagent:**
   ...
   - Agent: `stelow-product-testing-ai-code` or equivalent
...
- **stelow-product-shape-up**: Produces the shaped proposal
- **stelow-product-plan-critique**: Reviews the proposal before tech planning
