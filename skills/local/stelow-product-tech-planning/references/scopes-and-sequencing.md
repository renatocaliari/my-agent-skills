...
**How it works (see `stelow-product-scope-executor`):**
...
```
optimization scope → stelow-product-scope-executor → subagent → **optimization goal** (see `references/cli-tools/goals.md` → Optimization Goals)
```

The stelow-product-scope-executor delegates to a subagent with an acceptance contract that includes benchmark verify commands.
