...
import (
	...
	todosFeature "northstar/features/todos"
	voicetrainingFeature "northstar/features/voice-training"
	whiteboardFeature "northstar/features/whiteboard"
...
func SetupRoutes(ctx context.Context, router chi.Router, sessionStore *sessions.CookieStore, ns *embeddednats.Server) (err error) {
	...
	if err := errors.Join(
		indexFeature.SetupRoutes(router),
		todosFeature.SetupRoutes(router, sessionStore, ns),
		counterFeature.SetupRoutes(router),
		monitorFeature.SetupRoutes(router),
		sortableFeature.SetupRoutes(router),
		reverseFeature.SetupRoutes(router),
		whiteboardFeature.SetupRoutes(router, sessionStore, ns),
		voicetrainingFeature.SetupRoutes(router, ns),
	); err != nil {
